...
-- Archivo de consultas del sistema.
-- Incluye consultas básicas, intermedias y avanzadas
-- para análisis de reservaciones, facturación y ocupación.

-- ==========================================
--                CONSULTAS
-- ==========================================

-- 1. Mostrar todos los hoteles registrados
SELECT * FROM hotel;

-- 2. Mostrar habitaciones disponibles actualmente
SELECT
    id_habitacion,
    numero,
    piso,
...
-- 3. Mostrar reservaciones junto con huésped y habitación
SELECT
    r.id_reservacion,
...
-- 5. Calcular el total del hospedaje según cantidad de noches
SELECT
    r.id_reservacion,
    hu.nombre,
    hu.apellido,
...
SELECT
    ...
    hu.nombre,
    hu.apellido,
    COALESCE(SUM(f.total_final), 0) AS total_gastado
FROM huesped hu
LEFT JOIN reservacion r ON hu.id_huesped = r.id_huesped
LEFT JOIN factura f ON r.id_reservacion = f.id_reservacion
...
SELECT
    ...
    COALESCE(SUM(cs.cantidad), 0) AS total_cantidad
FROM servicio s
LEFT JOIN consumo_servicio cs ON s.id_servicio = cs.id_servicio
GROUP BY s.nombre_servicio
ORDER BY total_cantidad DESC;

-- 9. Cantidad de reservaciones por hotel
SELECT
    ho.nombre AS hotel,
...
-- 10. Reservaciones por tipo de habitación
SELECT
    th.descripcion AS tipo_habitacion,
    COUNT(r.id_reservacion) AS total_reservaciones
...
-- 12. Detalle de servicios cobrados por factura
SELECT
    f.id_factura,
    s.nombre_servicio,
    d.cantidad,
...
-- 16. Ingresos generados por hotel
SELECT
    ho.nombre AS hotel,
...
-- 17. Habitaciones más reservadas
SELECT
    ha.id_habitacion,
    ha.numero,
    ho.nombre AS hotel,
...
-- 19. Check-in y check-out programados para hoy
SELECT
    r.id_reservacion,
    hu.nombre,
    hu.apellido,
...
SELECT
    ...
    COALESCE((SELECT SUM(total_final) FROM factura), 0) AS ingresos_totales;
